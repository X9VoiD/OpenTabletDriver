...
namespace OpenTabletDriver.UI.ViewModels
{
    ...
    {
        internal TabletService Handler { get; }
        ...
        public TabletViewModel(TabletService handler)
        {
            Handler = handler;

            Handler.PropertyChanged += (sender, e) =>
            {
                if (e.PropertyName == nameof(TabletService.Profile))
                {
                ...
                else if (e.PropertyName == nameof(TabletService.TabletState))
                {
